  ...
  const grid = [
    {
      ...
      name: "Опыт",
      desc: "Работаем в сфере туризма более 12 лет. Отправили более 5 млн туристов на отдых.",
    },
    {
      icon: (
        <svg
      ...
      <div className="flex flex-row justify-around">
        {grid.map((el, index) => (
          <div key={index} className="w-1/5 flex flex-col items-center">
            <div className="bg-gray-400 bg-opacity-30 m-5 p-4 rounded-icon-circle  border-gray-600">
              {el.icon}
            </div>
            <p className="text-2xl font-semibold mt-3 mb-6">{el.name}</p>
